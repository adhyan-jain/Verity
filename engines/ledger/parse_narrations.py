...
def extract_payment_rail(narration: str) -> str:
    ...
    if "RTGS" in narration_upper:
        ...
        "CHQ" in narration_upper
        or "CHEQUE" in narration_upper
        or "CLEARING" in narration_upper
    ):
        return "CHEQUE"
    elif (
        "CASH" in narration_upper
        or "CDM" in narration_upper
        or "ATM" in narration_upper
    ):
        return "CASH_ATM"
...
def extract_counterparty(narration: str) -> str | None:
    """
    ...
    cleaned = re.sub(
        r"^(TRF (?:FROM|FRM|TO)|NEFT|RTGS|UPI/CR|UPI/DR|FDRL/)\s*",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
...
def parse_bank_ledger(
    file_path: str = "data/raw/bank.xlsx", force_reload: bool = False
) -> pd.DataFrame:
    """
    ...
    print(
        f"Parsing raw ledger file: {file_path} (this may take a few seconds on first load)..."
    )
    ...
    withdrawal_amt = pd.to_numeric(raw_df["WITHDRAWAL AMT"], errors="coerce").fillna(
        0.0
    )
    ...
    print(
        f"Successfully cached {len(parsed_df)} parsed transactions to {PARQUET_CACHE_FILE}"
    )
...
def get_account_transactions(
    account_id: str, df: pd.DataFrame | None = None
) -> pd.DataFrame:
    """
        ...
        summaries.append(
            {
                ...
                "top_rail": group["payment_rail"].mode().iloc[0]
                if not group["payment_rail"].empty
                else "OTHER",
            }
        )
    return summaries
...
if __name__ == "__main__":
    ...
    for s in summaries:
        print(
            f"  Account {s['account_id']}: {s['total_transactions']} txns, Balance Range: [{s['min_balance']:,.2f} to {s['max_balance']:,.2f}]"
        )
