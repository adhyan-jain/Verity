...
try:
    from .parse_narrations import parse_bank_ledger, get_account_transactions, get_all_account_summaries
    from .anomalies import detect_all_ledger_anomalies, detect_balance_breaks, detect_timing_spikes, detect_reversal_outliers
    from .reconcile import compute_account_baseline
    from .timeline import build_account_timeline, TIMELINE_CACHE_FILE
...
_LEDGER_DF: Optional[pd.DataFrame] = None
_TIMELINES_CACHE: Optional[Dict[str, Any]] = None
...
def walk_ledger_graph(
    ...
    clean_acc = str(account_id).replace("'", "").strip()
    df = get_df()
    acc_df = df[df["account_id"] == clean_acc].sort_values("datetime", ascending=False).head(limit)
    ...
    for idx, (_, row) in enumerate(acc_df.iterrows()):
        steps.append({
            "step_index": idx + 1,
            "from_account": clean_acc if row["direction"] == "debit" else (row["counterparty"] or "EXTERNAL_ENTITY"),
            "to_account": (row["counterparty"] or "EXTERNAL_ENTITY") if row["direction"] == "debit" else clean_acc,
            "tier": "real_ledger",
