...
def compute_account_baseline(account_df: pd.DataFrame) -> dict[str, Any]:
    ...
    p95_daily_velocity = (
        float(np.percentile(daily_counts, 95))
        if not daily_counts.empty
        else mean_daily_velocity
    )
...
def compute_all_account_baselines(
...
if __name__ == "__main__":
    ...
    for acc_id, b in baselines.items():
        print(
            f"Account {acc_id} -> Daily Vel: {b['velocity']['mean_daily']:.1f} txns/day (P95: {b['velocity']['p95_daily']:.1f}), Reversal Rate: {b['reversals']['baseline_reversal_rate']:.3f}"
        )
