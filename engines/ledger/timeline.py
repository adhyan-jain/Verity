...
try:
    ...
    from .anomalies import detect_balance_breaks, detect_timing_spikes, detect_reversal_outliers
...
def build_account_timeline(
    ...
    acc_df["is_debit_num"] = np.where(acc_df["direction"] == "debit", acc_df["amount"], 0.0)
    acc_df["is_credit_num"] = np.where(acc_df["direction"] == "credit", acc_df["amount"], 0.0)
    ...
    daily_agg = acc_df.groupby("date_str").agg(
        tx_count=("id", "count"),
    ...
    combined_txns = pd.concat([evidence_txns, sampled_normal]).sort_values("datetime").reset_index(drop=True)
    ...
    return {
        ...
        "tier": "real_ledger",
        "total_transactions": int(len(acc_df)),
        "summary": {
...
if __name__ == "__main__":
    ...
    for acc, t in timelines.items():
        print(f"Account {acc} -> Total Txns: {t['total_transactions']}, Anomalies: {len(t['anomalies'])}, Density Points: {len(t['density_curve'])}")
