...
try:
    ...
    from .reconcile import compute_account_baseline, compute_all_account_baselines
except (ImportError, ValueError):
    from parse_narrations import parse_bank_ledger, get_account_transactions
    from reconcile import compute_account_baseline, compute_all_account_baselines


def detect_balance_breaks(df: pd.DataFrame, baseline: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    ...
    std_balance = max(1.0, baseline["balance"]["std"])
    ...
    for i, step in enumerate(steps):
        ...
        if step < -100000 and (step < mean_step - 3.5 * std_step):
            ...
            anomalies.append({
                ...
                "evidence_transaction_ids": [str(tx_prev["id"]), str(tx_curr["id"])]
            })
...
def detect_timing_spikes(df: pd.DataFrame, baseline: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
...
def detect_reversal_outliers(df: pd.DataFrame, baseline: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    ...
    for _, row in df.iterrows():
        ...
        if amt in active_pool:
            active_pool[amt] = [item for item in active_pool[amt] if (dt - item["datetime"]).total_seconds() <= 172800]
...
def detect_all_ledger_anomalies(df: Optional[pd.DataFrame] = None) -> List[Dict[str, Any]]:
    """
...
if __name__ == "__main__":
    ...
    for a in anomalies[:8]:
        print(f"  [{a['anomaly_type'].upper()}] Account {a['account_id']} (Severity: {a['severity']}) -> Window: {a['window_start']} to {a['window_end']}, Obs: {a['observed_value']} (Baseline: {a['baseline_value']})")
