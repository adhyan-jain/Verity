    ...
    for collector in G.nodes():
        ...
        for i in range(len(in_edges)):
            ...
            for j in range(i + 1, len(in_edges)):
                time_diff = (in_edges[j][1]["datetime"] - start_time).total_seconds() / 3600.0
            ...
            if len(unique_senders) >= 3 and total_window_amount >= (threshold * 1.8):
                ...
                already_flagged = any(collector in f["involved_accounts"] for f in flags)
...
def detect_round_tripping(
    ...
    def dfs_cycle(start_node: str, curr_node: str, path: List[str], edges_used: List[str], first_time: Any, first_amt: float, prev_time: Any, prev_amt: float):
        ...
        for _, next_node, k, edge_data in G.out_edges(curr_node, keys=True, data=True):
            ...
            if next_node == start_node and len(path) >= 3:
                ...
                if total_duration_hours <= max_cycle_hours and volume_retention >= min_volume_retention:
                    cycle_nodes = path
            ...
            # Continue path if not repeating intermediate nodes
            if next_node not in path and len(path) < max_cycle_length:
                # Inter-hop time gap limit
                if (curr_time - first_time).total_seconds() / 3600.0 <= max_cycle_hours:
                    dfs_cycle(
                        start_node,
                        next_node,
                        path + [next_node],
...
def detect_rapid_layering(
    ...
    def dfs_layer(curr_node: str, current_path: List[str], current_edges: List[str], prev_time: Any, prev_amount: float):
        if len(current_edges) >= min_hops:
            path_key = tuple(current_path)
            already_flagged = any(tuple(f["involved_accounts"]) == path_key for f in flags)
            if not already_flagged:
                flags.append({
        ...
        for _, next_node, k, edge_data in G.out_edges(curr_node, keys=True, data=True):
            ...
            dfs_layer(next_node, current_path + [next_node], current_edges + [edge_data["id"]], curr_time, curr_amount)

    for start_node in G.nodes():
        for _, first_dst, k, first_edge in G.out_edges(start_node, keys=True, data=True):
            dfs_layer(
                first_dst,
