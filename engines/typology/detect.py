...
import json
try:
    from .fatf_rules import build_networkx_graph, detect_structuring, detect_round_tripping, detect_rapid_layering
...
def detect_all_typologies(network_data: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    """
...
def evaluate_adversarial_set(adversarial_file: str = "data/synthetic/adversarial_set.json") -> Dict[str, Any]:
    """
    ...
    for case in test_cases:
        ...
        G = build_networkx_graph(edges)
        detected = detect_structuring(G) + detect_round_tripping(G) + detect_rapid_layering(G)
...
if __name__ == "__main__":
    ...
    for f in flags:
        print(f"  [{f['typology'].upper()}] {f['flag_id']} (Conf: {f['confidence']}) -> Accounts: {f['involved_accounts']}, Evt Txns: {f['evidence_transaction_ids']}")
        
    print("\nEvaluating Adversarial Hold-Out Test Set...")
    eval_res = evaluate_adversarial_set()
    print(f"Accuracy: {eval_res['accuracy']*100:.1f}% ({eval_res['passed_cases']}/{eval_res['total_test_cases']} passed)")
    for d in eval_res["details"]:
        status_icon = "PASS" if d["passed"] else "FAIL"
        print(f"  [{status_icon}] {d['test_id']}: {d['description']} -> Expected: {d['expected_verdict']}, Got: {d['actual_verdict']}")
