...
def run_checks() -> None:
    ...
    resp = requests.get("http://127.0.0.1:8001/api/v1/fraud/transaction/TX-CARD-9842", headers=fraud_headers, timeout=10)
    check("get_transaction returns 200", resp.status_code == 200, f"(got {resp.status_code})")
    body = resp.json()
    check("transaction id echoes back", body.get("id") == "TX-CARD-9842")
    check("tier is real_card", body.get("tier") == "real_card")

    resp = requests.get("http://127.0.0.1:8001/api/v1/fraud/explain/TX-CARD-9842", headers=fraud_headers, timeout=15)
    check("get_shap_explanation returns 200", resp.status_code == 200, f"(got {resp.status_code})")
    explanation = resp.json()
    check("risk_score present and in [0,1]", 0.0 <= explanation.get("risk_score", -1) <= 1.0)
    check("top_factors non-empty", len(explanation.get("top_factors", [])) > 0)

    print("\n1b. Fraud engine — rejects requests without a valid API key")
    resp = requests.get("http://127.0.0.1:8001/api/v1/fraud/transaction/TX-CARD-9842", timeout=10)
    ...
    if account_id:
        resp = requests.get(f"http://127.0.0.1:8002/api/v1/ledger/timeline/{account_id}", timeout=45)
        check("get_timeline returns 200 for a real account", resp.status_code == 200, f"(got {resp.status_code})")
        timeline = resp.json()
        check("timeline has transactions", len(timeline.get("transactions", [])) > 0)

        resp = requests.get(f"http://127.0.0.1:8002/api/v1/ledger/walk/{account_id}", timeout=10)
        check("walk_ledger_graph returns 200", resp.status_code == 200, f"(got {resp.status_code})")

    print("\n3. Typology engine — synthetic FATF network + flags")
    resp = requests.get("http://127.0.0.1:8003/api/v1/typology/network", timeout=10)
    check("get_synthetic_network returns 200", resp.status_code == 200, f"(got {resp.status_code})")
    network = resp.json()
    check("network has nodes and edges", len(network.get("nodes", [])) > 0 and len(network.get("edges", [])) > 0)

    resp = requests.get("http://127.0.0.1:8003/api/v1/typology/flags", timeout=10)
    check("list_typology_flags returns 200", resp.status_code == 200, f"(got {resp.status_code})")
    ...
    resp = requests.post(
        ...
        json={"case_id": "SMOKE-CARD-001", "transaction_id": "TX-CARD-9842", "tier_origin": "real_card"},
        timeout=30,
    )
    check("investigate returns 200", resp.status_code == 200, f"(got {resp.status_code})")
    case = resp.json()
    ...
    resp = requests.post(
        "http://127.0.0.1:8000/api/v1/agent/investigate",
        json={"case_id": "SMOKE-SYNTH-001", "transaction_id": "TX-SYNTH-5501", "tier_origin": "synthetic_network"},
        timeout=30,
    )
    ...
    check(
        ...
        any(e.get("tool_called") == "walk_graph" for e in synth_case.get("trace_events", [])),
    )

    resp = requests.post(
        "http://127.0.0.1:8000/api/v1/agent/investigate",
        json={"case_id": "SMOKE-LEDGER-001", "transaction_id": "TX-LEDGER-3011", "tier_origin": "real_ledger"},
        timeout=30,
    )
    ...
    check("counterfactual returns 200", resp.status_code == 200, f"(got {resp.status_code})")
    cf = resp.json()
    check(
    ...
    chat = resp.json()
    check("cached benchmark answer is used and disclosed", chat.get("is_fallback") is True and bool(chat.get("fallback_notice")))
