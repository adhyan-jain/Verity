...
def get_transaction(transaction_id: str) -> Dict[str, Any]:
    ...
    if VERITY_ENV == "live":
        try:
            ...
            resp = requests.get(
                f"{FRAUD_API_URL}/transaction/{transaction_id}", timeout=TOOL_TIMEOUT
            )
...
def get_shap_explanation(transaction_id: str) -> Dict[str, Any]:
    ...
    if VERITY_ENV == "live":
        try:
            resp = requests.get(
                f"{FRAUD_API_URL}/explain/{transaction_id}", timeout=TOOL_TIMEOUT
            )
            if resp.status_code == 200:
                return resp.json()
        except requests.exceptions.RequestException as e:
...
def walk_graph(
    ...
    if VERITY_ENV == "live":
        try:
            if tier == "real_ledger":
                resp = requests.get(
                    f"{LEDGER_API_URL}/walk/{account_id}", timeout=TOOL_TIMEOUT
                )
            else:
                resp = requests.get(
...
def counterfactual(
    ...
    if VERITY_ENV == "live":
        try:
            ...
            resp = requests.post(
                f"{FRAUD_API_URL}/counterfactual", json=payload, timeout=TOOL_TIMEOUT
            )
            if resp.status_code == 200:
                return resp.json()
        except requests.exceptions.RequestException as e:
            logger.info(
                "Live counterfactual API unavailable, falling back to mock: %s", e
            )
    ...
    if "Amount" in parameter_overrides or "amount" in parameter_overrides:
        new_amt = float(
            parameter_overrides.get("Amount", parameter_overrides.get("amount", 4850.0))
        )
        if new_amt <= 200.0:
            ...
            recalculated_risk_score = round(
                min(0.95, max(0.15, (new_amt / 5000.0) * 0.85)), 2
            )
            recalculated_verdict = (
                "flagged" if recalculated_risk_score >= 0.5 else "clear"
            )
    ...
    if "Time" in parameter_overrides or "time" in parameter_overrides:
        time_val = str(
            parameter_overrides.get("Time", parameter_overrides.get("time", ""))
        )
